{{-- page formulaire de modification d’une reserver --}}

@extends('layouts.main') {{-- herite du layout principal --}}

@section('title', 'Modifier la reservation') {{-- titre personnalise --}}
        ...
        <div class="mb-3">
            ...
            <select id="table_id" name="table_id" class="form-control"> {{-- choix entre table deja existante --}}
                @foreach($tables as $table)
                    <option value="{{ $table->id }}">{{ $table->designation }}</option> {{-- on recupere l id --}}
                @endforeach
            </select>
        </div>
